...
import { Player } from "./video/player";
import { AudioRecorder } from "react-audio-voice-recorder";
import VideoStreamPlayer from "./video/video-stream-player";
...
export default function VideoProcessor() {
  ...
  const [volume, setVolume] = useState(1);
  const [videoSrc, setVideoSrc] = useState<string | null>("");
  const [videoStream, setVideoStream] = useState<ReadableStream | null>(null);
        ...
        fetch(`/progress/${clientId}`)
          ...
          .then((data) => {
            if (data.status === "inactive") {
              setError("Processing stopped due to inactivity.");
              setIsLoading(false);
        ...
        <Card>
          ...
          <CardContent>
            ...
            {videoStream && (
              <Card className="mt-4">
                <CardHeader>
                  <CardTitle>Processed Video Stream</CardTitle>
                </CardHeader>
                <CardContent>
